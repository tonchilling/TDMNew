(function (window, undefined) {
    ...
    var DBF = function (url, callback) {
        ...
        else if (url instanceof Blob || url instanceof File)
            this.handleFile(url, callback);
        else
        ...
        readRecords: function () {
            ...
            for (var index = 0; index < numRecords; index++) {
                ...
                for (var i = 0; i < numFields; i++) {
                    field = fields[i]
                    var val = s.readString(field.fieldLength).trim()
                    try {
                        val = decodeURIComponent(escape(val)).trim();
                    } catch (e) {
                    }
                    record[field.name] = val.replace(/\u0000/g, '');
                }
