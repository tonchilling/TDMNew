    ...
    clearDropDown: function(eleName) {
        ...
        switch (eleName) {
            case "ddlProvince":
                ...
                break;
            case "ddlDistrict":
                firstOption = "เลือกอำเภอ";
                break;
            case "ddlSubdistrict":
                firstOption = "เลือกตำบล";
                break;
        }
        $(name).empty();
...
var mapApi = {
    getProvincesByRegion: function(regionId, fnSuccess) {
        ...
        $.get("/api/Map/GetProvincesByRegion", {
            id: regionId
        }, function(provinces) {
            fnSuccess(provinces);
        });
    ...
    getDistrictsByProvince: function(provinceId, fnSuccess) {
        ...
        $.get("/api/Map/GetDistrictsByProvince/", {
            id: provinceId
        }, function(data) {
            fnSuccess(data);
        });
    ...
    getSubDistrictsByDistrict: function(districtId, fnSuccess) {
        ...
        $.get("/api/Map/GetSubDistrictsByDistrict/", {
            id: districtId
        }, function(data) {
            fnSuccess(data);
        });
    ...
    getProvinceShapByCode: function(provinceCode, fnSuccess) {
        $.get("/api/Address/GetProvinceShapeBy", {
            code: provinceCode
        }, function(data) {
            fnSuccess(data);
        });
    ...
    getProvinceShapeByID: function(provinceID, fnSuccess) {
        $.get("/api/Map/GetProvinceShapeByID", {
            id: provinceID
        }, function(data) {
            fnSuccess(data);
        });
    ...
    getProvinceShapeByRegion: function(regionId, fnSuccess) {
        $.get("/api/Map/GetProvinceShapeByRegion", {
            id: regionId
        }, function(data) {
            fnSuccess(data);
        });
    ...
    getDistrictShapeByID: function(districtId, fnSuccess) {
        $.get("/api/Map/GetDistrictShapeByID", {
            id: districtId
        }, function(data) {
            fnSuccess(data);
        });
    ...
    getDistrictShapeByProvince: function(provinceId, fnSuccess) {
        $.get("/api/Map/GetDistrictShapeByProvince", {
            id: provinceId
        }, function(data) {
            fnSuccess(data);
        });
    ...
    getSubDistrictShapeByID: function(subDistrictId, fnSuccess) {
        $.get("/api/Map/GetSubDistrictShapeByID", {
            id: subDistrictId
        }, function(data) {
            fnSuccess(data);
        });
    ...
    getSubDistrictShapeByDistrict: function(districtId, fnSuccess) {
        $.get("/api/Map/getSubDistrictShapeByDistrict", {
            id: districtId
        }, function(data) {
            fnSuccess(data);
        });
...
$(document).ready(function() {
    ...
    begin = cookieDatas.indexOf(prefixCookie);
    end = -1;
    if (begin > -1) {
        end = document.cookie.indexOf(";", begin);
        color = '';
        if (end > -1) {
            color = unescape(cookieDatas.substring(begin + 1 + prefixCookie.length, end));
        } else {
            color = unescape(cookieDatas);
        }
        color = color.replace(prefixCookie + "=", "");
        $('body').removeAttr('id').attr('id', 'body-' + color);
    }
    $('body').fadeIn();
    $('.chartBarNormal').css({
        'width': $(controlBS[0]).width()
    });
                ...
                optionChart = {
                    ...
                    yAxis: [{
                        ...
                        data: [{
                            ...
                            value: 'ใต้',
                            labelColor: 'yellow'
                        }, 'กลาง', 'เหนือ', 'ตะวันตก', 'ตะวันออก', 'ตะวันออกเฉียงเหนือ']
                    }],
                    series: [{
                        name: 'ราคาประเมินรวม',
                        type: 'bar',
    ...
    $('.color-block').click(function() {
        $('body').removeAttr('id').attr('id', 'body-' + $(this).attr('data-color'));
        date = new Date();
        date.setTime(date.getTime() + (3650 * 24 * 60 * 60 * 1000));
        expires = "expires=" + date.toUTCString();
        document.cookie = prefixCookie + "=" + $(this).attr('data-color') + "; " + expires;
    });
    $('#example').DataTable();
